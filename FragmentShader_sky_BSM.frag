...
{
	vec3 secondRayPosBSM = p - vec3(0.0, g_radius, 0.0); // the baseground has 200000 height, we should subtract it ( too large value may cause precision problem)
	vec4 pSunView = sunView * vec4(secondRayPosBSM, 1.0);
	float pDistance = -pSunView.z; // view space look into negative z

	vec4 pSunViewNDC = sunProj * sunView * vec4(secondRayPosBSM, 1.0);
	vec3 pSunProj = pSunViewNDC.xyz / pSunViewNDC.w;
	vec2 shadowMapCoords = (pSunProj.xy / 2) + 0.5;
	vec3 BSMtex = texture(BSM, shadowMapCoords).rgb;

	float opticalDist = min(BSMtex.b, BSMtex.g * max(0, pDistance - BSMtex.r));
