...
float getLight(vec3 p, float maxDist, float extinction, vec3 viewDir, vec3 secondRayDir)
{
	vec3 secondRayPosESM = p - vec3(0.0, g_radius, 0.0); // the baseground has 200000 height, we should subtract it ( too large value may cause precision problem)
	vec4 pSunView = sunView * vec4(secondRayPosESM, 1.0);
	float pDistance = -pSunView.z; // view space look into negative z
	float normalizedDist = pDistance / 15000; // 15000 from the ESM (the maximam trace distance)
	float expDist = exp(-extinction * normalizedDist); // 30 from ESM, the extinction
...
vec4 march(const vec3 pos, const vec3 end, vec3 dir, const int numSamples) 
{
	...
	for (int i = 0; i < numSamples; i++) // sample points until the max number
	{
		...
		{ 
			counter++;
			float beers = getLight(p, 15000, extinction, dir, secondRayDir);	
